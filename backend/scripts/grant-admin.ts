...
const main = async (): Promise<void> => {
  ...
  try {
    const updated = await setUserRoleByEmail(actor, email, "admin");
    console.log(`Granted admin role to ${updated.email} (${updated.id}).`);
  } finally {
...
main().catch((error) => {
  if (error instanceof Error) {
    ...
    console.error("Unknown error while granting admin role.");
  }
